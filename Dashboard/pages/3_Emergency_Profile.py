...
from modules.profile_store import load_profile, save_profile
...
st.set_page_config(page_title="RoadSoS | Rider Profile & SOS", page_icon="🪖", layout="wide", initial_sidebar_state="expanded")
inject_global_css()
init_location_state()
sidebar_brand()
render_theme_toggle()
...
page_header("🪖 Rider Profile & SOS", "Medical handoff packet for responders and bystanders.", "QR READY", RED)
...
with left:
    with st.form("profile"):
        ...
        conditions = st.text_area("Medical Conditions", profile["conditions"])
        ...
        lat = st.text_input("Last known latitude", lat_default)
        lon = st.text_input("Last known longitude", lon_default)
...
<div class="profile-card">
  ...
  <div style="margin-bottom:0.9rem;">
    <span class="chip">Allergies: {safe_allergies}</span>
    <span class="chip">Conditions: {safe_conditions}</span>
  </div>
  <div style="color:{c["MUTED"]};font-size:0.75rem;font-family:'Outfit';text-transform:uppercase;letter-spacing:0.06em;font-weight:700;margin-bottom:0.25rem;">📞 Emergency contact</div>
...
if submitted:
    ...
    alert_banner("safe", "Profile preview updated. The SOS packet below reflects the current form values.")

st.markdown("<h3 style='margin-top:1.5rem;'>SOS Packet Preview</h3>", unsafe_allow_html=True)
...
<div class="profile-card" style="height: 100%;">
  <div style="color:{RED};font-weight:800;margin-bottom:0.9rem;font-family:'Outfit';text-transform:uppercase;letter-spacing:0.08em;font-size:0.95rem;">WHO / Red Cross crash response protocol</div>
  <div style="color:{c["TEXT"]};line-height:1.7;font-size:0.9rem;font-family:'Inter';">
    <b>1.</b> Secure the scene first. Move traffic away, switch off ignition, and call emergency services.<br>
    <b>2.</b> Do not move the rider unless fire, traffic, or flooding creates immediate danger.<br>
    <b>3.</b> Check response, airway, breathing, and severe bleeding. Start CPR only if trained and needed.<br>
    <b>4.</b> Stabilize the head and neck manually. Treat suspected spine injury as real until responders arrive.<br>
    <b>5.</b> Apply firm direct pressure to heavy bleeding with clean cloth or gauze.<br>
    <b>6.</b> Keep the rider warm, calm, and still. Share the QR profile and GPS location with responders.
  </div>
</div>
""",
        unsafe_allow_html=True,
...
""",
    ...
    st.caption("Scan for the offline emergency medical packet")
